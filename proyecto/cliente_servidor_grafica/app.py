...
u = 'FIS200' #tsv(b'gAAAAABmW5Lxj7w68-gCjn6X594pWsQFywkyb72fpQ-dLSHYD66D7oTzLrTiXT6cCNJ1GfVQ4rmuOepxoBs_wqSR-mGs2D8enw==')
...
h = '192.168.35.148' #tsv(b'gAAAAABmW5L5JqmQn7tCq-yZLyFcXDYt65j8H-WXt4z-Fz8DwBB-X8kQE33EZo2Yiyikvm3aHK83nd2YD2mo8ecIiJx3-8wAw6qrTm3NOA7hbgOapXl2EGxVnCRMthDGMGbpw2edf1-7')
P = '3306' #int(tsv(b'gAAAAABmW5L79yTu3nD1ouMESJ_KXq0wGHnlaZEyb4gQpQdlEV7zjoHwBQRmZY4-5eEHp-FK-Dwdr_0Z_Z7e1Pd6avR1wowI5Q=='))
d = '8-980-1739'#tsv(b'gAAAAABmW5L83JtdOdRxjNmkT8-2-sUhdSTp9LjU7IGahvCGObZD2ewvidwnLRtZmlcn36FKEe_f_DjD5CYRYwcb4wqnMVQJMg==')
            ...
            if connection.is_connected():
                ...
                for row in new_data:
                    for column, value in row.items():
                        if column != 'ns' and column != 'finsert':
                            updated_column_data[column].append(value)
                        if column == 'finsert':
                            updated_column_data[column].append(str(value))
    ...
    try:
        if user_id not in user_data:
            user_data[user_id] = {
                ...
                'message_queue': Queue(),
                'sql_query': f"SELECT * FROM (SELECT tempC, ID, tiempo, finsert FROM `{nombre_tabla}` ORDER BY id DESC) AS sub ORDER BY id ASC;",
                'str_a': f"SELECT * FROM (SELECT tempC, ID, tiempo, finsert FROM `{nombre_tabla}` ORDER BY id DESC) AS sub ORDER BY id ASC;",
                'str_b': f"SELECT tempC, ID, tiempo, finsert FROM `{nombre_tabla}` ORDER BY id DESC LIMIT 1;",
